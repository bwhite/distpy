...
cdef extern from "hamming_aux.h":
    void hamdist_cmap_lut16(np.uint8_t *xs, np.uint8_t *ys, np.uint32_t *out, int size, int num_xs, int num_ys)
...
cdef class Hamming(object):
    ...
    cpdef np.ndarray[np.uint32_t, ndim=2, mode='c'] cdist(self, np.ndarray[np.uint8_t, ndim=2, mode='c'] a,
        ...
        cdef np.ndarray out = np.zeros((a.shape[0], b.shape[0]), dtype=np.uint32)
        hamdist_cmap_lut16(<np.uint8_t *>a.data, <np.uint8_t *>b.data, <np.uint32_t *>out.data,
    ...
    cpdef int dist(self,
        ...
        hamdist_cmap_lut16(<np.uint8_t *>v0.data, <np.uint8_t *>v1.data, &out, v0.size, 1, 1)
        return out

    cpdef np.ndarray[np.uint32_t, ndim=2, mode='c'] knn(self,
        ...
        cdef np.ndarray[np.uint32_t, ndim=1, mode='c'] dists = np.zeros(neighbors.shape[0], dtype=np.uint32)
        hamdist_cmap_lut16(<np.uint8_t *>neighbors.data, <np.uint8_t *>vector.data, <np.uint32_t *>dists.data,
